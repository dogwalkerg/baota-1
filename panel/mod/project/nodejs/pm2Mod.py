...
class main(NodeJs):
        ...
        try:
            ...
            for jl in get_jlist:
                ...
                if jl.get("pm2_env").get("exec_mode") == "fork_mode":
                    ...
                    if cluster_mode.get("name") == jl.get("name"):
                        tmp["mode"] = "fork_mode"
                        cluster_mode["data"].append(tmp)
    ...
    def start(self, mode: str = None, id: str = None, name: str = None, *args, **kwargs):
        ...
        if mode == "fork_mode":
            stdout, stderr = public.ExecShell("sudo {} start {}".format(self.pm2_cmd, id))
        elif mode == "cluster_mode":
            stdout, stderr = public.ExecShell("sudo {} start {}".format(self.pm2_cmd, name))
        else:
            return public.returnResult(False, 'mode参数错误', code=5)
        # 忽略无效警告
        if stderr and "sudo:" not in stderr:
    ...
    def structure_start_script(self, get):
        ...
        pm2_file = os.path.join(self.pm2_config_path, get.project_name, "ecosystem.config.cjs")
        command = '''{last_env}
export NODE_PROJECT_NAME="{project_name}"
export HOME=/root
{pm2_cmd} start {pm2_file}'''.format(
            last_env=last_env,
            project_name=get.project_name,
        ...
        if not os.path.exists(os.path.join(self.pm2_config_path, get.project_name)):
            os.makedirs(os.path.join(self.pm2_config_path, get.project_name), 493)
        public.writeFile(pm2_file, get.ecosystem)
        public.writeFile(os.path.join(get.project_cwd, "{}_start.sh".format(get.project_name)), command)
        get.config_file = pm2_file
