    ...
    def check_spring_boot_args(get) -> Union[Dict, str]:
        ...
        if project_cmd.startswith("./") or project_cmd.startswith("../"):
            ...
            if os.path.exists(project_cmd_bin):
                ...
                return "启动命令的二进制文件为相对路径，这可能导致服务启动失败，请改为绝对路径在使用"
...
echo $! > {pid_file}'''.format(
    ...
    def create_tomcat_project(self, get):
        ...
        tomcat_host_name = \
        (config["domains"][0] if isinstance(config["domains"], List) else config["domains"]).split(":")[0]
        if not tomcat.add_host(
                name = tomcat_host_name, path = config["project_path"], context_path = config[
                    "project_name"]
                ):
    ...
    def create_project(self, get: Get):
        ...
        if not os.path.exists(project_path):
            public.print_log(project_jar)
            public.print_log(project_path)
        ...
        # *添加反向代理
        # !统一为domains
        if (get.get("domains", "") and get.domains[0] != "") and get.get("target_dir", "") not in ("/", ""):
            p_get = public.dict_obj()
            p_get.site_name = get.project_name
            p_get.proxy_dir = get.proxy_dir
            p_get.proxy_port = get.port
            target_path = get.get("target_dir", "/")
            if target_path == "/" and project_type == 1:
                target_path += get.project_name
            p_get.rewrite = {
                "status": True,
                "src_path": get.proxy_dir,
            ...
            if get.get("static_dir", ""):
                ...
                if not static_res["status"]:
                    self.remove_project(get)
                    return json_response(False, "关联静态资源失败")
    ...
    def _fix_bt_tomcat(project_data: dict):
        ...
        if tomcat.is_error_config_xml():
            tomcat.repair_config([{"name": project_config["name"], "path": project_data["path"]}])
        else:
            tomcat.add_host(project_config["name"], project_data["path"])
            tomcat.save_config_xml()
        tomcat.restart()
