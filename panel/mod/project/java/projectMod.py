...
echo $! > {pid_file}'''.format(
    ...
    def modify_site_tomcat_project(self, get, project_data: Optional[dict] = None):
        ...
        if flag:
            tomcat.set_service(user=project_config.get("run_user", "root"), auto_start=project_config.get("auth", False))
        ...
        for project_data in data['data']:
            ...
            if not tomcat:
                ...
                project_data["server_config_error"] = tomcat.check_config(
                    project_config["project_name"], project_data["path"], project_config.get("access_mode", "domain")
                )
