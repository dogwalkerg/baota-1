...
location ~ [^/]\.php(/|$) {{
    ...
    def set_site_path(self, get):
        ...
        if get.path is None: return public.returnResult(False, "path参数不能为空！")
        get.site_name = get.get("site_name", None)
        if get.site_name is None: return public.returnResult(False, "site_name参数不能为空！")
        ...
        get.proxy_json_conf["site_path"] = get.path

        dp.sql("docker_sites").where("id=?", (get.id,)).setField("path", get.path)
        ...
        if "type" in get.proxy_json_conf and get.proxy_json_conf["type"] == "php":
            ...
            public.ExecShell("sed -i 's,^SITE_PATH=.*,SITE_PATH={},' {}/.env".format(get.path, service_info["project_path"]))
    ...
    def set_site_run_path(self, get):
        ...
        if "type" in get.proxy_json_conf and get.proxy_json_conf["type"] == "php":
            service_info = json.loads(dp.sql("docker_sites").where("id=?", (get.id,)).getField("service_info"))
            public.ExecShell("sed -i 's,^SITE_PATH=.*,SITE_PATH={},' {}/.env".format(os.path.join(get.proxy_json_conf["site_path"], get.proxy_json_conf["run_path"]), service_info["project_path"]))
            public.ExecShell("docker-compose -f {} up -d".format(os.path.join(service_info["project_path"], "docker-compose.yml")))
    ...
    def GetSiteDomains(self, get):
        ...
        from panelSite import panelSite
        n_list = []
        domains = dp.sql("docker_domain").where("pid=?", get.id).field("name,id").select()
        for domain in domains:
            tmp = {'id': domain['id'], 'name': domain['name'],'binding':False,'apply_ssl': 1 }
            tmp.update(panelSite().get_domian_dns_status(domain['name']))
