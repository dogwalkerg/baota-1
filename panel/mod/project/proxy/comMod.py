...
server {{
    ...
    def structure_nginx(self, get):
        ...
        self._init_proxy_conf["proxy_cache"]["cache_zone"] = get.site_name.replace(".", "_") + "_cache" if get.site_port == "80" else "{}_{}_cache".format(get.site_name.replace(".", "_"), get.site_port)
    ...
    def create(self, get):
        ...
        if not os.path.exists(get.site_path + "/proxy_cache_dir"):
            return public.returnResult(status=False, msg="创建缓存目录失败，请尝试手动创建{}是否成功，如果不成功请联系宝塔运维".format(get.site_path + "/proxy_cache_dir"))
