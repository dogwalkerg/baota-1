...
def clean_iptables():
    ...
    清空恶意IP列表
    """
    public.M('firewall_malicious_ip').where('1=?', (1,)).delete()
    if __isUfw or not __isFirewalld:
...
def process_iptables(ban_ip_list):
    ...
    for ip in ban_ip_list:
        _string = _string + "add {} {}\n".format("malicious_ipset", ip)
    tmp_file.write(_string)
    tmp_file.close()

    public.ExecShell('ipset create malicious_ipset hash:ip maxelem 1000000 timeout 86400;'
                     'ipset restore -f {}'.format(firewall_malicious_path))

    if __isUfw and not __isFirewalld: # ufw防火墙和未知类型的
        public.ExecShell('iptables -I INPUT -m set --match-set malicious_ipset src -j DROP')
    else:
        public.ExecShell('firewall-cmd --permanent --zone=public --direct --add-rule ipv4 filter INPUT 0 -m set --match-set malicious_ipset src -j DROP')
    reload_firewall()
