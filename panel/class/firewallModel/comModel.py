...
class main(Base):
    ...
    def set_status(self, get):
        ...
        if get.status == '1':
            ...
            get.port = public.readFile(_panel_port_file).strip(" ").strip("\n")
    ...
    def set_ip_rule(self, get):
        ...
        if get.address != "all" and "\n" in get.address:
            ...
            if get.zone.lower() == "trusted" and not self._isFirewalld:
                return public.returnMsg(False, 'trusted区域只支持firewalld防火墙')
            if get.chain == "INPUT":
                result = self.firewall.rich_rules(info=info, operation=get.operation)
            else:
                result = self.firewall.output_rich_rules(info=info, operation=get.operation)
    ...
    def ip_rules_list(self, get):
        ...
        return self.structure_ip_return_data(list_address, ip_db, get)
