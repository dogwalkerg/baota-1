...
""" % (get.port, get.port, get.webname, get.domain)
    ...
    def GetSiteDomains(self, get):
        ...
        for domain in domains:
            ...
            tmp.update(self.get_domian_dns_status(domain['name']))
    ...
    def get_domian_dns_status(self, domain):
        try:
            from sslModel import dataModel
            data = dataModel.main().get_domain_dns_config(public.dict_obj())
            for config in data:
                if domain == config['name']:
                    return {'dns_status': config['status'], 'root_domain_id': config['domain_id']}
            return {'dns_status': 0, 'root_domain_id': ""}
        except:
            return {'dns_status': 0, 'root_domain_id': ""}
...
#PROXY-END%s''' % (get.proxydir, get.proxydir, get.proxysite, get.proxydir,
  ...
  "zblog"
    ...
    def ToBackup(self, get):
        ...
        tmps = '/tmp/site_backup_{}.log'.format(id)
...
""".format("" if recursive not in ['true', True] else "\nreal_ip_recursive on;"))
            public.serviceReload()
