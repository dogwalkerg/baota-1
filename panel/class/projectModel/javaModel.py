...
'''.format(jsvc_make_path, jdk_path, tomcat_path + "/bin/jsvc")
    ...
    def send_cmd(self, get):
        ...
        return public.returnMsg(True, '{} {} {} -jar -Xmx1024M -Xms256M  {} --server.port={}'.format(project_jdk, lib_path_cmd, jmx_cmd, project_jar,
                                                                                                    port))
        ...
        if 'search' in get:
            ...
            if type_id is None:
                ...
                data['data'] = public.M('sites').where('project_type=? AND (name LIKE ? OR ps LIKE ?) AND type_id = ?',
                                                       ('Java', search, search, type_id)).limit(
                    data['shift'] + ',' + data['row']).order(get.order).select()
    ...
    def fix_project(self, get):
        ...
        if project_config['java_type'] == 'duli':
            if not os.path.exists(self.__bttomcat_path + "/tomcat_bak{}/conf/server.xml".format(
                project_config['tomcat_version'])): return public.returnMsg(False, '修复失败当前Tomcat版本未安装')
            tomcat_list = ["7", "8", "9"]
            ...
            if log_file is None:
                try:
                    log_file = public.readFile(
                        "/usr/local/bttomcat/tomcat%s/conf/logpath.conf" % version) + f'/catalina-daemon-{version}.out'
                except:
            ...
            if project_config['java_type'] == 'springboot':
                ...
                project_state = self.get_duli_run_state(project_name=project_name,
                                                        bt_tomcat_web='/usr/local/bttomcat/tomcat{}'.format(
                                                            project_config['tomcat_version']), neizhi=True)
    ...
    def multi_remove_project(self, get):
        ...
        projects = public.M('sites').where(f'project_type=? AND name in ({",".join(["?"] * len(pjnames))})',
                                           ('Java', *pjnames)).select()
        if not projects:
        ...
        public.M('sites').where(f'project_type=? AND name in ({",".join(["?"] * len(pjnames))})',
                                ('Java', *pjnames)).delete()
...
'''.format(
    ...
    def for_split(self, logsplit, project):
        ...
        if project['project_config']['java_type'] == 'springboot':
            ...
            if site_tomcat:
                tomcat = site_tomcat(project["name"])
                if tomcat:
                    log_file = tomcat.log_file
            if log_file is None:
        ...
        if project_info['project_config']['java_type'] == 'duli':
            ...
            if os.path.exists(s_file):
                ...
                if not os.path.exists(target_file):
                    ...
                    os.remove(s_file)
            if not self._change_daemom_sh(self.__site_path + project_info["name"] + "/bin/daemon.sh", _path[:-1],
    ...
    def get_status_info(self, get):
        ...
        try:
            ...
            jmxConnection = JMXConnection(connection_uri=jmx_url, jmx_username=username, jmx_password=password,
                                          java_path=java_path)
