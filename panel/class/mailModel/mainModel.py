...
'''
    ...
    def set_mail_forward(self, args):
        ...
        if self.M('alias').where('address=?', args.user).count() > 0:
            return self.return_msg(public.returnMsg(False, '被转发用户已经存在'))
        # 检查域名是否存在
        if self.M('domain').where('domain=?', args.domain).count() <= 0:
            return self.return_msg(public.returnMsg(False, '邮件服务器中不存在该域名'))
        # 检查被转发用户是否存在邮局内
        if self.M('mailbox').where(
                'username=?', args.user).count() <= 0 and args.user[0] != '@':
            return self.return_msg(public.returnMsg(False, '邮件服务器中不存在被转发用户'))
        # 换行符替换为逗号
        ...
        return self.return_msg(public.returnMsg(True, '邮件转发添加成功'))

    def edit_mail_forward(self, args):
        if self.M('alias').where('address=?', args.user).count() == 0:
            return self.return_msg(public.returnMsg(False, '被转发用户不存在'))
        # 换行符替换为逗号
        tmp = args.forward_user.split('\\n')
        if "\n" in tmp[0]:
            tmp = tmp[0].split('\n')
        ...
        return self.return_msg(public.returnMsg(True, '邮件转发编辑成功'))

    def delete_mail_forward(self, args):
        if self.M('alias').where('address=?', args.user).count() == 0:
            return self.return_msg(public.returnMsg(False, '转发用户不存在'))
        self.M('alias').where('address=?', args.user).delete()
        return self.return_msg(public.returnMsg(True, '邮件转发删除成功'))
